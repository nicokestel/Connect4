...
from .common import BoardPiece, SavedState, PlayerAction, get_non_full_columns, PLAYER2, PLAYER1, \
    initialize_game_state


ohe = OneHotEncoder(categories=[[-1, 0, 1]] * 42).fit(initialize_game_state().flatten().reshape(1, -1))

mlp_model_file = os.path.join(os.path.dirname(__file__), 'MLP_MODEL.pkl')
